...
stack = rmakers.stack(
    ...
    rmakers.before_grace_container(
        [0, 2, 3], abjad.select().logical_ties(pitched=True)
    ),
)
time_signatures = [
...
pitches = [
    abjad.NumberedPitch(x) + (math.sin(x))
...
ts_staff_2 = abjad.Staff(
    skips_two, lilypond_type="TimeSignatureContext", name="globalcontext"
)
